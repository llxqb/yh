...
//				MainActivity.this.finish();
				Toast.makeText(MainActivity.this, "不是后退键，是不是很生气啊", 0).show();
			}
		});
	...
	private void isOver(boolean isWin) {
		...
		if (isWin) {
			...
			if(level % 3==0){
				message = "你的分数为："
						+ (int) (Math.pow(BOMBS[level % 3], 2) / Math.sqrt(time))+"  蛮厉害的嘛，小雅美女";
//				SystemClock.sleep(1000);
				startActivity(new Intent(MainActivity.this,FriendActivity.class));
			}else if(level % 3==1){
				message = "你的分数为："
						+ (int) (Math.pow(BOMBS[level % 3], 2) / Math.sqrt(time))+"  好戏在后头,一定要记得完成第三关哦";
				startActivity(new Intent(MainActivity.this,FamilyActivity.class));

			}else if(level % 3==2){
				message = "你的分数为："
						+ (int) (Math.pow(BOMBS[level % 3], 2) / Math.sqrt(time))+"   如果我是那雨滴的话... 那么，我能够像把不曾交会的    天空与大地连接起来那样... 把某人的心串联起来吗？";
